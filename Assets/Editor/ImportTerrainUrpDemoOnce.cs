...
/// <summary>
/// One-shot importer for Asset Store product 213197 (Unity Terrain - URP Demo Scene).
/// </summary>
[InitializeOnLoad]
public static class ImportTerrainUrpDemoOnce
{
    public const long ProductId = 213197;
    const string StatusAbs = @"C:\Users\dongx\FirstFPS\Logs\import-terrain-status.txt";
    const string PrefsImported = "FirstFPS.ImportTerrainUrpDemo.Imported";
    const string SessionKey = "FirstFPS.ImportTerrainUrpDemo.Running";
    ...
    static ImportTerrainUrpDemoOnce()
    {
        WriteStatus("STATIC_CTOR prefsImported=" + EditorPrefs.GetBool(PrefsImported, false));
    ...
    [MenuItem("GameMesh/Import Unity Terrain URP Demo")]
    ...
    static async Task RunAsync()
    {
        ...
        try
        {
            ...
            {
                WriteStatus("FAIL no packman token");
                SessionState.SetBool(SessionKey, false);
                return;
            }
            ...
            {
                WriteStatus("FAIL product 213197 not downloadable. Add Unity Terrain - URP Demo Scene to My Assets first");
                SessionState.SetBool(SessionKey, false);
                return;
            }
            ...
            var packageName = ExtractJsonString(downloadJson, "filename_safe_package_name") ?? "UnityTerrainURPDemo";
            ...
            WriteStatus("DOWNLOADING " + packageName + " keyLen=" + (key?.Length ?? 0));
    ...
    static async Task<string> DownloadAndDecryptAsync(string url, string key, string token)
    {
        ...
        var encryptedPath = Path.Combine(logsDir, "terrain-urp-encrypted.bin");
        var decryptedPath = Path.Combine(logsDir, "UnityTerrainURPDemo.unitypackage");
    ...
    static bool TryDecryptStreaming(string encryptedPath, string decryptedPath, string key)
    {
        ...
        foreach (var attempt in attempts)
        {
            ...
            {
                WriteStatus("DECRYPT_OK " + attempt.name + " skip=" + attempt.skip + " size=" + new FileInfo(decryptedPath).Length);
                return true;
            }
        }
    ...
    static Task<string> GetAuthCodeAsync(string clientId)
    {
        ...
        try
        {
            ...
            typeof(ImportTerrainUrpDemoOnce).GetMethod(nameof(InvokeAuth), BindingFlags.NonPublic | BindingFlags.Static)
    ...
    static async Task<bool> DownloadFileAsync(string url, string destFile, string token)
    {
        try
        {
            using var client = NewClient(token, TimeSpan.FromHours(3));
    ...
    static bool DirectoryExistsImported()
    {
        ...
        foreach (var name in new[] { "TerrainDemoScene_URP", "Unity Terrain Demo", "TerrainDemoScene" })
        {
    ...
    static void WriteStatus(string line)
    {
        try
        {
            ...
            Debug.Log("[ImportTerrainUrp] " + line);
        }
        catch
        {
            // ignored
