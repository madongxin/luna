...
/// <summary>
/// One-shot importer for Unity Starter Assets Third Person (URP).
/// Tries product 196526 then 267961.
/// </summary>
[InitializeOnLoad]
public static class ImportStarterAssetsThirdPersonOnce
{
    static readonly long[] ProductIds = { 196526, 267961 };
    const string StatusAbs = @"C:\Users\dongx\FirstFPS\Logs\import-starter-tp-status.txt";
    const string PrefsImported = "FirstFPS.ImportStarterAssetsThirdPerson.Imported";
    const string SessionKey = "FirstFPS.ImportStarterAssetsThirdPerson.Running";
    ...
    static ImportStarterAssetsThirdPersonOnce()
    {
    ...
    [MenuItem("GameMesh/Import Starter Assets Third Person")]
    ...
    static async Task RunAsync()
    {
        ...
        try
        {
            ...
            if (downloadJson == null)
            {
                WriteStatus("FAIL add Starter Assets - ThirdPerson URP (196526) or Character Controllers URP (267961) to My Assets first");
                SessionState.SetBool(SessionKey, false);
                return;
            }
            ...
            var packageName = ExtractJsonString(downloadJson, "filename_safe_package_name") ?? "StarterAssetsThirdPerson";
    ...
    static void OnImported(string packageName)
    {
        ...
        SessionState.SetBool(SessionKey, false);
        EditorApplication.delayCall += RefreshCatalogSafe;
    }

    static void RefreshCatalogSafe()
    {
        var type = Type.GetType("GameMesh.Editor.PlayableScenePlayerInstaller, GameMesh.Editor");
        type?.GetMethod("RefreshCatalog", BindingFlags.Public | BindingFlags.Static)?.Invoke(null, null);
        WriteStatus("CATALOG refresh invoked type=" + (type != null));
    }
    ...
    static async Task<string> DownloadAndDecryptAsync(string url, string key, string token)
    {
        ...
        var encryptedPath = Path.Combine(logsDir, "starter-tp-encrypted.bin");
        var decryptedPath = Path.Combine(logsDir, "StarterAssetsThirdPerson.unitypackage");
    ...
    static Task<string> GetAuthCodeAsync(string clientId)
    {
        ...
        try
        {
            ...
            typeof(ImportStarterAssetsThirdPersonOnce).GetMethod(nameof(InvokeAuth), BindingFlags.NonPublic | BindingFlags.Static)
    ...
    static bool DirectoryExistsImported()
    {
        ...
        return Directory.Exists(Path.Combine(assets, "StarterAssets"))
               || Directory.Exists(Path.Combine(assets, "Starter Assets"));
    }
    ...
    static void WriteStatus(string line)
    {
        try
        {
            ...
            Debug.Log("[ImportStarterTP] " + line);
        }
        catch
        {
            // ignored
